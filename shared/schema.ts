...
export const chatbotSettings = pgTable("chatbot_settings", {
  ...
  welcomeMessage: text("welcome_message").default("أهلاً! 👋 أنا المساعد الذكي لشركتنا. يسعدني مساعدتك. ممكن تعرفني باسمك الكريم؟"),
  botName: varchar("bot_name").default("المساعد الذكي"),
  companyName: varchar("company_name").default("شركتنا"),
  botRole: varchar("bot_role").default("مستشار مبيعات"),
  botPersonality: text("bot_personality").default("أنت مستشار مبيعات مصري محترف وودود. بتتكلم بالمصري بشكل طبيعي. بتساعد العملاء وبتجمع بياناتهم بطريقة محترمة."),
  botMission: text("bot_mission").default("جمع بيانات العميل الكاملة وترشيح الخدمات المناسبة قبل تحويله للمندوب."),
